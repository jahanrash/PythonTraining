...
def backTest(datas, closei, changei):
    ...
    for row in datas.iterrows():
        try:
            ...
            currentPercentChange = round(((currentValuation - startingCapital)/startingCapital) * 100)
...
'''
stock_list = ['a', 'aa', 'aapl', 'abbv', 'abc', 'abt', 'ace', 'aci', 'acn', 'aci', 'act', 'adbe']
...
res = results()
res.plot(label='wholistic performance')
